...
void sFLASH_WriteBuffer(const uint8_t* pBuffer, uint32_t WriteAddr, uint16_t NumByteToWrite)
{
  ...
  sFLASH_SendByte(sFLASH_CMD_AAI);		//Aca hay que mandarle el comando de AAI
  ...
  while ((NumByteToWrite--) > 0)
  {
	  ...
	  {
		  sFLASH_CS_LOW();
		  sFLASH_SendByte(sFLASH_CMD_AAI);
	  }

    /*!< Send the current byte */
    sFLASH_SendByte(~(*pBuffer)); 		//hay que mandar dos bytes y desp esperar a que se escriban, para mandar los dos siguientes.
    ...
    else 	// Si el numero de bytes a escribir es impar, el ultimo byte que escribo es cero
    	sFLASH_SendByte(~(0));
    /* Aca hay que esperar a que termine de escribir los bytes*/

    sFLASH_CS_HIGH();
    /* wait for the two bytes to be written in memory and the, write the other two */
    sFLASH_WaitForWriteEnd();
    NoFirstTime=1;
...
uint8_t sFLASH_ReadByte(void)
{
 // return (sFLASH_SendByte(sFLASH_DUMMY_BYTE));
  /*!< Loop while DR register in not emplty */
   ...
   SPI_I2S_SendData(sFLASH_SPI, sFLASH_DUMMY_BYTE);
