...
//		if (move_window(fs, fs->dirbase + (bc / SS(fs)))) break;
		if (move_window(fs, fs->database + (bc / SS(fs)))) break;
...
//			res = move_window(fs, fs->dirbase + (bc / SS(fs)));
			res = move_window(fs, fs->database + (bc / SS(fs)));
			if (res != FR_OK) break;
			p = &fs->win[bc % SS(fs)];
...
)
{
	...
	mem_set(tbl, 0, SS(fs));
	mem_cpy(tbl, "\xEB\xFE\x90" "MSWIN4.1", 11);/* Boot jump code, OEM name */
	i = SS(fs);								/* Sector size */
	...
	if (fmt == FS_FAT32) {
		...
		mem_cpy(tbl+BS_VolLab, "GPS SPORT  " "FAT12   ", 19);	/* Volume label, FAT signature */
	}
